...
test("callLlm: openrouter -- sends system+user messages, returns trimmed content", async () => {
  ...
  await withMockFetch(
    ...
    async () => {
      const text = await callLlm("system prompt", "user prompt", { provider: "openrouter", model: "m", apiKey: "key" });
...
test("callLlm: openrouter -- non-ok response throws with the error message", async () => {
  await withMockFetch(
    (async () => new Response(JSON.stringify({ error: { message: "rate limited" } }), { status: 429 })) as typeof fetch,
    async () => {
      await assert.rejects(() => callLlm("s", "u", { provider: "openrouter", model: "m", apiKey: "key" }), /rate limited/);
    },
  );
});

test("callLlmMessages: openrouter -- passes the full message list through unmodified (few-shot support)", async () => {
  let capturedBody: unknown;
  const messages = [
  ...
  await withMockFetch(
    ...
    async () => {
      const text = await callLlmMessages(messages, { provider: "openrouter", model: "m", apiKey: "key" });
...
test("callLlm: bedrock -- routes to bedrock-mantle with the right path/model, extracts trimmed content", async () => {
  let capturedUrl = "";
  let capturedAuth = "";
  await withBedrockToken("test-token", () =>
    withMockFetch(
      ...
      async () => {
        const text = await callLlm("system prompt", "user prompt", { provider: "bedrock", model: "google.gemma-4-31b", region: "us-east-1" });
...
test("callLlm: bedrock -- unmapped model defaults to the plain /v1 path", async () => {
  let capturedUrl = "";
  await withBedrockToken("test-token", () =>
    withMockFetch(
      (async (url: string) => {
      ...
      () => callLlm("s", "u", { provider: "bedrock", model: "zai.glm-5", region: "us-east-1" }),
    ),
  );
  assert.equal(capturedUrl, "https://bedrock-mantle.us-east-1.api.aws/v1/chat/completions");
});

test("callLlm: bedrock -- no AWS_BEARER_TOKEN_BEDROCK set throws before any network call", async () => {
  let called = false;
  await withBedrockToken(undefined, () =>
    withMockFetch(
      ...
      async () => {
        await assert.rejects(() => callLlm("s", "u", { provider: "bedrock", model: "m", region: "us-east-1" }), /no credentials/);
      },
    ),
  );
...
test("callLlm: bedrock -- no region (option or env) throws before any network call", async () => {
  ...
  try {
    await withBedrockToken("test-token", async () => {
      await assert.rejects(() => callLlm("s", "u", { provider: "bedrock", model: "m" }), /no region/);
    });
  } finally {
...
test("callLlm: bedrock -- non-ok response throws with the error message (caller's retry logic is responsible for catching it)", async () => {
  await withBedrockToken("test-token", () =>
    withMockFetch(
      (async () => new Response(JSON.stringify({ error: { message: "Operation not allowed" } }), { status: 403 })) as typeof fetch,
      async () => {
        await assert.rejects(() => callLlm("s", "u", { provider: "bedrock", model: "m", region: "us-east-1" }), /Operation not allowed/);
      },
    ),
  );
});

test("callLlm: bedrock -- missing/unknown content resolves to empty string, not a throw", async () => {
  const text = await withBedrockToken("test-token", () =>
    withMockFetch(
      (async () => new Response(JSON.stringify({ choices: [] }), { status: 200 })) as typeof fetch,
      () => callLlm("s", "u", { provider: "bedrock", model: "m", region: "us-east-1" }),
    ),
  );
  assert.equal(text, "");
});
