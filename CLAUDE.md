...
`gate-overrides.json`, the multi-server auth-token config, captured sessions
under `sessions/`, and `lib/` — where the zero-touch bootstrap `npm install
--prefix`es the CLI so the launch marker never points into npm's evictable
cache) lives under `~/.twing/` — never inside this repo's working tree, so
none of it was ever something `.gitignore` needed to name. (Older installs
may still have a `~/Library/LaunchAgents/dev.twing.daemon.plist` or
