...
// childCoordinator is one candidate project a plan resolves to
// (discoverPlanCoordinators, plan_paths.go).
type childCoordinator struct {
	// DirName is the repo root's base name, used only for labelling in
	// messages and logs. Path matching is done by real resolution against
	// RepoRoot, never by comparing this string -- see pathsForRepo.
	DirName   string
	ServerURL string
	RepoRoot  string
}
