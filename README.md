...
If `--server` differs from the committed coordinator, `init` leaves the file
unchanged by default. Use `twing init --server <url> --replace-server` only
when you intend to repoint the whole repository, then review and commit the
change. `--replace-server` is rejected with `--unattended`.
...
twing has no way to do that for you. Non-GitHub-hosted and no-auth setup can
write the hook in one command with `twing init --enable-enforcement`; repos
founded before this existed can still run `twing project enable-enforcement`.
...
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `twing init [--server <url>] [--replace-server]`      | One-time setup per machine: discover/confirm the coordinator, authenticate, install/wire the hook, start the daemon. `--replace-server` deliberately replaces a conflicting committed coordinator and cannot be used unattended. Safe to re-run. |
...
| Command                                                                                            | What it does                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `twing init [--server <url>] [--replace-server] [--invite <code>] [--no-auth] [--no-github] [--enable-enforcement] [--unattended]`           | One-time setup per machine: discovers/bootstraps the coordinator, authenticates (GitHub-verified join/found by default for a GitHub-hosted repo; `--invite` to redeem one instead; `--no-github` to skip straight to the old "no cached PAT" error; `--no-auth` to declare this coordinator has no identity verification at all), hook install, hook wiring (including the design gate), daemon start. `--replace-server` deliberately replaces a conflicting committed coordinator and cannot be used unattended. `--enable-enforcement` explicitly writes the committed bootstrap hook, including for no-auth setup; it cannot be used unattended. `--unattended` is what the committed bootstrap hook runs: never prompts, never opens a browser, and skips the OS-service install and global wiring -- see "Zero-touch onboarding". Safe to re-run. |
